...
router.findAllIssues = function (req, res) {
...
router.findById = function (req, res) {
    ...
    issues.find({ _id: req.params.id }, function (err, issue) {
        ...
        } else res.send(JSON.stringify(issue, null, 5));
        // return the donation
    });
};

router.findByCate = function (req, res) {
...
router.findByStatus = function (req, res) {
    res.setHeader('Content-Type', 'application/json');
...
router.addIssue = function (req, res) {
    ...
    issue.save(function (err) {
        if (err) res.json({ message: 'Issue NOT Added!', errmsg: err });else res.json({ message: 'Issue Successfully Added!', data: issue });
    });
};
...
router.updateStatus = function (req, res) {
    ...
    issues.findById(req.params.id, function (err, issue) {
        if (err) res.json({ message: 'Donation NOT Found!', errmsg: err });else {
            issue.status = req.params.status;
            issue.save(function (err) {
                if (err) res.json({ message: 'Issue NOT Updated to Solved!', errmsg: err });else res.json({ message: 'Issue Successfully Set Solved!', data: issue });
            });
        }
    });
};

router.deleteIssue = function (req, res) {
    issues.findByIdAndRemove(req.params.id, function (err) {
        if (err) {
            res.status(404);
            res.json({ message: 'Issue NOT DELETED!', errmsg: err });
        } else res.json({ message: 'Issue Successfully Deleted!' });
    });
};
